...
В данном компоненте мы использовали деконструкцию переменной. Это очень удобно, потому что уменьшает количество кода.
Пример:
```javascript
...
c === props.c; // true
```
Тут есть условие, что имена переменных должны совпадать названием свойств объекта.
...
Тоесть, примерно так:
```jsx
<Lesson index={ index } name={ lesson.name } rooms={ lesson.room } key={ index } highlight={ isHighlighted(lesson) } />
```
