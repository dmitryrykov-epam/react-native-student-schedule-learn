...
Функция `renderWeek` уже создана, но не готова до конца. Изучите ее, потому что вам придется писать похожие функции в других компонентах.

А теперь мы "исправим" эту функцию. Вам нужно изменить значение переменной `changedElement` на компонент `DaySchedule` и передать ему параметры из переменной `day`.

Коспонент `DaySchedule` должен иметь следующие атрибуты:
- `key` - служебный атрибут должен быть уникальным, желательно присвоить значение инекса элемента массива (переменная `i`)
- `isHighlighted` - присвоить значение `day.isHighlighted`
- `date` - `day.date`
- `lessons` - `day.lessons`
...
Ну вот, компонент `Lesson` готов! Осталось посмотреть на полученный результат.
...
Продолжение есть вот здесь: [Анимация в `React Native`](./animation.md)